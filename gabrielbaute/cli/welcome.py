...
from gabrielbaute.gabrielcv import get_banner
...
def show_welcome():
    ...
    table.add_column("Tipo", style="bold blue")
    table.add_column("Descripción", style="green")

    for cmd, meta in get_available_commands().items():
        table.add_row(f"`{cmd}`", meta["tag"], meta["desc"])
